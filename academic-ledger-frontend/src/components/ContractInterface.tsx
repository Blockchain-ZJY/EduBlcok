...
const ContractInterface: React.FC = () => {
    ...
    showInputModal(
      ...
      async (address) => {
        if (!ethers.utils.isAddress(address)) {
          showConfirmModal('错误', '请输入有效的以太坊地址', () => {}, 'error');
          return;
        }
        ...
        // 第二步：输入专业
          ...
          async (prog) => {
            if (!prog.trim()) {
              showConfirmModal('错误', '专业名称不能为空', () => {}, 'error');
              return;
            }
            program = prog.trim();
            ...
            // 第三步：输入学位
              ...
              async (lv) => {
                if (!lv.trim()) {
                  showConfirmModal('错误', '学位层级不能为空', () => {}, 'error');
                  return;
                }
                level = lv.trim();
                      ...
                      async (u) => {
                        ...
                        showInputModal(
                          ...
                          async (hash) => {
                            ...
                            try {
                              const docHashBytes32 = docHash 
                                ? docHash.startsWith('0x') 
                                  ? docHash 
                                  : '0x' + docHash.padEnd(64, '0')
                              ...
                              showConfirmModal('成功', `证书颁发成功！\n证书ID: ${certId}`, () => {}, 'success');
                                } catch (error: any) {
                                  console.error('颁发失败:', error);
                              showConfirmModal('错误', '颁发失败: ' + (error.message || '未知错误'), () => {}, 'error');
                                } finally {
                                  setLoading(false);
                                }
                              },
                          { placeholder: '0x... 或留空', required: false }
                            );
                        },
                      { placeholder: 'ipfs://... 或留空', required: false }
                      );
                  },
                  { placeholder: '0', required: true }
                );
            },
              { placeholder: '例如：学士、硕士、博士', required: true }
          );
          },
          { placeholder: '例如：计算机科学', required: true }
        );
      },
  ...
  const _issueCertificateWithIPFS = async () => {
    ...
    if (!ipfsConnected) {
      showConfirmModal(
        '提示',
        'IPFS未连接，将无法上传文件。\n你可以继续使用传统方式颁发证书，或者先配置Pinata API密钥。',
        () => {},
        'warning'
      );
    }
    ...
    let expiresAt = '';
    let description = '';
    let uploadedFileIPFS: string | null = null;
    ...
    showInputModal(
      ...
      async (address) => {
        ...
        try {
          ...
          showConfirmModal('警告', '该地址未注册为学生，是否继续？', async () => {
            proceedToStep2();
          }, 'warning');
          return;
        }
                    ...
                    async (desc) => {
                      ...
                      showInputModal(
                        ...
                        async (exp) => {
                          ...
                          // 如果用户上传了文件，先上传到IPFS
                          if (uploadedFile && ipfsConnected) {
                            setUploadingToIPFS(true);
                            try {
                              const fileResult = await pinataService.uploadFile(uploadedFile, {
                                name: `${studentAddress.substring(0, 10)}-certificate-document`,
                                keyvalues: {
                                  student: studentAddress.substring(0, 10),
                                  type: 'certificate-document'
                                }
                              });
                              uploadedFileIPFS = fileResult.IpfsHash;
                            } catch (error: any) {
                              console.error('文件上传到IPFS失败:', error);
                              showConfirmModal('警告', '文件上传失败，是否继续颁发证书？\n' + error.message, async () => {
                                await proceedToIssue();
                              }, 'warning');
                          ...
                          async function proceedToIssue() {
                            if (!contract) {
                              showConfirmModal('错误', '合约未连接', () => {}, 'error');
                              return;
                            }
        ...
        setLoading(true);
                            ...
                            try {
                              ...
                              const institutionInfo = await contract.getInstitution(account);
                              
                              // 创建元数据
                              const metadata: FileMetadata = {
                                studentName: studentInfo.name || '未知',
                                studentId: studentInfo.studentId || studentAddress,
                                institutionName: institutionInfo.name || '未知',
                                program: program,
                                level: level,
                                issuedDate: new Date().toISOString(),
                                description: description || `${level}学位证书`,
                                documentIPFS: uploadedFileIPFS || undefined
                              };
                              
                              // 上传元数据到IPFS
                              let metadataIPFS = '';
                              let docHash = ethers.constants.HashZero;
                              
                              if (ipfsConnected) {
                                try {
                                  metadataIPFS = await pinataService.uploadCertificateMetadata(metadata);
                                  // 生成文档哈希（使用IPFS哈希）
                                  docHash = ethers.utils.id(metadataIPFS);
        } catch (error: any) {
                                  console.error('元数据上传到IPFS失败:', error);
                                  showConfirmModal('警告', '元数据上传失败，将使用空URI\n' + error.message, () => {}, 'warning');
                                }
                              }
                              
                              const uri = metadataIPFS ? `ipfs://${metadataIPFS}` : '';
                              ...
                              let successMessage = `证书颁发成功！\n\n证书ID: ${certId}`;
                              if (metadataIPFS) {
                                successMessage += `\n\n📄 元数据IPFS: ${metadataIPFS}`;
                                successMessage += `\n🔗 查看: ${pinataService.getIPFSUrl(metadataIPFS)}`;
                              }
                              if (uploadedFileIPFS) {
                                successMessage += `\n\n📎 文档IPFS: ${uploadedFileIPFS}`;
                                successMessage += `\n🔗 查看: ${pinataService.getIPFSUrl(uploadedFileIPFS)}`;
                              }
                              
                              showConfirmModal('成功', successMessage, () => {
          ...
          setLoading(false);
                    ...
                    { placeholder: '例如：优秀毕业生', required: false, type: 'textarea' }
                  );
                },
                { placeholder: '例如：学士、硕士、博士', required: true }
              );
